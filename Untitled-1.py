...
if include_institutional:
    ...
    st.error("📅 重要限制：證交所三大法人買賣超 API 僅提供最近 10 年的資料，如需下載更早期的資料，請取消勾選此選項")
...
# 檢查三大法人資料的日期限制（僅提供最近 10 年）
institutional_date_warning = False
if include_institutional:
    # 計算 10 年前的日期（使用 365 * 10 天作為近似值）
    min_institutional_date = today - timedelta(days=365 * 10)

    if start_date < min_institutional_date:
        st.error(f"❌ 三大法人資料僅提供最近 10 年的資料（約 {min_institutional_date.strftime('%Y/%m/%d')} 之後）！您選擇的開始日期為 {start_date}，這將導致無法取得三大法人資料。")
        st.info("💡 解決方案：請將開始日期調整為最近 10 年內，或取消勾選「包含三大法人買賣超資訊」選項以下載純股價資料")
